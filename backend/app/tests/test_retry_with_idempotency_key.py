...
async def test_retry_with_same_key_returns_cached_response(db_engine, test_order):
   ...
   async with AsyncClient(app=app, base_url="http://test") as client:
      ...
      response_1 = await client.post(
         ...
         headers=headers)
      import json
      content = json.loads(response_1.content)["error"]
      
      assert response_1.status_code == 200, f"Статус ответа должен быть 200, получено: {response_1.status_code}, {content}"
...
async def test_same_key_different_payload_returns_conflict(db_engine, test_order):
   ...
   async with AsyncClient(app=app, base_url="http://test") as client:
      ...
      assert response_2.status_code == 409, "Ожидася статус 409, невалидное повторное использование idempotency key"
